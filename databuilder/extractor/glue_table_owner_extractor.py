...
class GlueTableOwnerExtractor(Extractor):
    """
    ...
    def init(self, conf):
        conf = conf.with_fallback(GlueTableOwnerExtractor.DEFAULT_CONFIG)
        self._cluster = '{}'.format(conf.get_string(GlueTableOwnerExtractor.CLUSTER_KEY))
    ...
    def get_scope(self):
        # type: () -> str
        return 'extractor.glue_table_owner_extractor'
    ...
    def _get_extract_iter(self):
        ...
        for row in self._get_raw_extract_iter():
            ...
            yield TableOwner(table_name=row['Name'],
                                      owners=[row['Owner']],
